    ...
    if (duplicates.length > 0) {
      ...
      duplicates.forEach(([productId, count], index) => {
        ...
        duplicateProducts.forEach((product, productIndex) => {
          console.log(
            `   ${productIndex + 1}. ${product.kind.toUpperCase()} - ${product.asset.symbol} - ${product.protocol.name}`
          )
          console.log(`      Market: ${product.protocol.name}`)
          console.log(
            `      Chain: ${product.protocol.chain.name} (${product.protocol.chain.id})`
          )
      ...
      console.log(
        `1. Même token sur différents marchés (ex: AaveV3Ethereum vs AaveV3EthereumLido)`
      )
