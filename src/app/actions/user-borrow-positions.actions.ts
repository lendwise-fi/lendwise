...
type ProtocolPositions = Record<ProtocolName, BorrowPosition[]>

export const loadUserBorrowPositions = cache(
    ...
    try {
      ...
      const results = await Promise.allSettled(
        protocolIds.map(async (protocolId) => {
          ...
          const positions = await protocolAdapter.getUserBorrowPositions({
            addresses,
          })
