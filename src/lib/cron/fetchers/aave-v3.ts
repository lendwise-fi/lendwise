...
import { MarketsApyQuery } from '@/lib/adapters/aave/v3/offchain/generated/graphql'
import { MARKETS_APY } from '@/lib/adapters/aave/v3/offchain/queries'
...
export async function fetchAaveV3Apy(
  ...
  for (const market of data.markets) {
    ...
    for (const reserve of market.reserves) {
      ...
      // Enrich APY with market-scoped Merkl incentive APR
      const tokenAddress = reserve.underlyingToken.address
      const supplyApy =
        baseSupplyApy +
        lookupIncentive(merklIncentives.supply, marketSlug, tokenAddress)
      const borrowApy =
        baseBorrowApy +
        lookupIncentive(merklIncentives.borrow, marketSlug, tokenAddress)
